    ...
    return __generator(this, function (_a) {
        return [2 /*return*/, new Promise(function (resolve, reject) {
                ...
                if (os.platform() === "darwin")
                    hwiFile = "HWI_MAC";
                var binariesPath = path.join(__dirname, '..', '..', './HWIs');
                var pathToHwi = path.resolve(path.join(binariesPath, hwiFile));
                // uncomment when testing newer versions of HWI
                // const pathToHwi = path.resolve(path.join(__dirname, '..', '..', '..', 'HWI', 'hwi.py'));
                execFile(pathToHwi, command, function (error, stdout, stderr) {
                    if (error) {
                        reject(error);
                    }
                    resolve(stdout);
