...
import { getAddressFromAccount, serializeTransactions } from '@lily/shared-server';
...
describe('accountMap', function () {
  ...
  test('Multisig - external getAddressFromAccount', () => {
    const addresses = [];
    for (let i = 0; i < 4; i++) {
      addresses.push(getAddressFromAccount(Multisig.config, `m/0/${i}`, networks.bitcoin));
    }
  ...
  test('Multisig - change getAddressFromAccount', () => {
    const addresses = [];
    for (let i = 0; i < 4; i++) {
      addresses.push(getAddressFromAccount(Multisig.config, `m/1/${i}`, networks.bitcoin));
    }
