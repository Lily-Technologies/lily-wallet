...
import { combinePsbts, validateAddress, validateSendAmount, validateTxForAccount, createUtxoMapFromUtxoArray, getFeeForMultisig, getFee, coinSelection, getPsbtFromText, getSignedFingerprintsFromPsbt, getSignedDevicesFromPsbt, createTransaction, truncateAddress } from '../../utils/send';
...
describe('Send Utils', () => {
    ...
    test('validateTxForAccount success', () => {
        ...
        expect(() => validateTxForAccount(psbt, {
            ...Mnemonic.config,
    ...
    test('combinePsbts', () => {
        ...
        expect(combinedPsbts1.txOutputs[0].address).toEqual(truncateAddress('bc1qf9gnplc9fvc6txpcfysfvcznfx6qmhksce0jjk'));
        expect(combinedPsbts1.txOutputs[0].value).toEqual(1000000);
        expect(combinedPsbts1.txOutputs[1].address).toEqual(Multisig.unusedChangeAddresses[0].address);
        ...
        expect(combinedPsbts2.txOutputs[0].address).toEqual(truncateAddress('bc1qf9gnplc9fvc6txpcfysfvcznfx6qmhksce0jjk'));
    ...
    test('createUtxoMapFromUtxoArray', () => {
        const UtxoMap = createUtxoMapFromUtxoArray(Multisig.availableUtxos);
        expect(Object.keys(UtxoMap).length).toEqual(Multisig.availableUtxos.length);
    });
    test('createMap', () => {
        const TransactionMap = createMap(Multisig.transactions, 'txid');
    ...
    test('getFee', () => {
        ...
        const fee = getFee(psbt, Multisig.transactions);
        expect(fee).toEqual(51308);
    });
    test('coinSelection', () => {
        const utxoMap = createUtxoMapFromUtxoArray(Multisig.availableUtxos);
        const { spendingUtxos, currentTotal } = coinSelection(1000000, Multisig.availableUtxos); // 0.01 BTC
    ...
    test('getPsbtFromText', () => {
        ...
        expect(psbt.txOutputs[0].address).toEqual(truncateAddress('bc1qf9gnplc9fvc6txpcfysfvcznfx6qmhksce0jjk'));
        expect(psbt.txOutputs[0].value).toEqual(1000000);
        expect(psbt.txOutputs[1].address).toEqual(Multisig.unusedChangeAddresses[0].address);
    ...
    test('getSignedDevicesFromPsbt', () => {
        const COMBINED_PSBTS_BASE64 = '';
        const psbt = Psbt.fromBase64(COMBINED_PSBTS_BASE64);
        const devices = getSignedDevicesFromPsbt(psbt, Multisig.config.extendedPublicKeys);
