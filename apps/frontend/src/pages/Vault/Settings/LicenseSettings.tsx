...
import { LilyLicense, File, VaultConfig, NodeConfigWithBlockchainInfo } from '@lily/types';
...
import { white, green400, green700, gray400, gray800, orange400, red500 } from 'src/utils/colors';
...
const LicenseSettings = ({ nodeConfig, openInModal, password, closeModal }: Props) => {
  ...
  const onLicenseUpload = (file: File) => {
    ...
    const licenseErrorMessage = getLicenseUploadErrorMessage(parsedFile as LilyLicense, nodeConfig);
    if (!licenseErrorMessage) {
      ...
      openInModal(<PurchaseLicenseSuccess config={updatedAccountConfig} nodeConfig={nodeConfig} />);
    } else {
      openInModal(<ErrorModal message={licenseErrorMessage} closeModal={closeModal} />);
    }
  };
  ...
  return (
    <SettingsTable.Wrapper>
      <SettingsTable.HeaderSection>
        <SettingsTable.HeaderTitle>License Information</SettingsTable.HeaderTitle>
      ...
      <SettingsTable.Row>
        <SettingsTable.KeyColumn>Approximate Expire Date</SettingsTable.KeyColumn>
        <SettingsTable.ValueColumn>
          <SettingsTable.ValueText></SettingsTable.ValueText>
      ...
      <SettingsTable.Row>
        <SettingsTable.KeyColumn>Payment Transaction ID</SettingsTable.KeyColumn>
        <SettingsTable.ValueColumn>
          <SettingsTable.ValueText></SettingsTable.ValueText>
          <SettingsTable.ValueAction>
      ...
      <Buttons>
        ...
        <LicenseButton background={green700} color={white} onClick={() => history.push(`purchase`)}>
          Renew License
        </LicenseButton>
      </Buttons>
      <FileUploader
